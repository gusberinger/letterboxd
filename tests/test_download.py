...
favorites = (
    "https://letterboxd.com/testuser_py/likes/films/",
    ["tt0422720", "tt0467406", "tt0045152", "tt3783958"],
)
...
class DownloadListTest(unittest.TestCase):
    ...
    def test_small_limit(self):
        url, ids = small_list
        result = download_list(url, 3)
        self.assertEqual(list(result), ids[:3])

    def test_large_limit(self):
        url, ids = small_list
        result = download_list(url, 40)
        self.assertEqual(list(result), ids[:40])


class ArgumentParseTest(unittest.TestCase):
    def test_limit_full(self):

        self.assertEqual(
            parse_args(
                ["https://letterboxd.com/testuser_py/list/larger-list/", "-limit", "10"]
            ),
            {
    ...
    def test_limit_abbrev(self):

        self.assertEqual(
            parse_args(
                ["https://letterboxd.com/testuser_py/list/larger-list/", "-l", "10"]
            ),
            {
                "url": "https://letterboxd.com/testuser_py/list/larger-list/",
                "limit": 10,
    ...
    def test_invalid(self):
        self.assertRaises(
            ValueError,
            lambda: parse_args(["htp://letterboxd.com/testuser_py/list/larger-list/"]),
        )
        self.assertRaises(
            ValueError,
            lambda: parse_args(["http://letterbod.com/testuser_py/list/larger-list/"]),
        )
